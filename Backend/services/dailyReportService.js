...
export const findReportById = async (report_id) => {
  ...
  try {
    ...
    if (rows[0]) {
      rows[0].images = parseImages(rows[0].images);
    }
    return rows[0];
  } finally {
    connection.release();
...
export const findReportsByUserInRange = async (
  user_id,
  start_date,
  end_date,
) => {
...
export const findAllReports = async () => {
  ...
  try {
    const [rows] = await connection.query(
      "SELECT * FROM daily_report ORDER BY report_date DESC",
    );
    return rows.map((row) => ({ ...row, images: parseImages(row.images) }));
  } finally {
    connection.release();
...
export const deleteReport = async (report_id) => {
  const connection = await pool.getConnection();
  try {
    await connection.query("DELETE FROM daily_report WHERE report_id = ?", [
      report_id,
    ]);
  } finally {
    connection.release();
...
export const findRecentTeamReports = async (manager_id, limit = 5) => {
  ...
  try {
    ...
    const [finalRows] = await connection.query(improvedQuery, [
      manager_id,
      limit,
    ]);
