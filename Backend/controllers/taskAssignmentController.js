...
export const assignTask = async (req, res) => {
  try {
    ...
    if (
      user.role === "MANAGER" &&
      project.assigned_manager_id !== user.user_id
    ) {
      return res
        .status(403)
        .json({ message: "You can only assign tasks in your projects" });
    }
    ...
    if (user.role === "MANAGER" && worker.manager_id !== user.user_id) {
      return res
        .status(403)
        .json({ message: "You can only assign tasks to your workers" });
    }

    const exists = await taskAssignmentService.assignmentExists(
      task_id,
      worker_id,
    );
    if (exists) {
      return res
        .status(400)
        .json({ message: "Worker is already assigned to this task" });
    }

    const assignment = await taskAssignmentService.createTaskAssignment(
      task_id,
      worker_id,
    );
  ...
  try {
    ...
    if (req.user.role === "MANAGER") {
      ...
      if (project.assigned_manager_id !== req.user.user_id) {
        return res
          .status(403)
          .json({ message: "You can only view assignments for your projects" });
      }
    }
    ...
    if (req.user.role === "WORKER") {
      return res
        .status(403)
        .json({ message: "Unauthorized to view task assignments" });
    }

    const assignments =
      await taskAssignmentService.findAssignmentsByTask(task_id);
...
export const getMyAssignments = async (req, res) => {
  try {
    ...
    const assignments =
      await taskAssignmentService.findAssignmentsByWorker(worker_id);
    res.json(assignments);
  } catch (error) {
    console.error(error);
...
export const getWorkerAssignments = async (req, res) => {
  try {
    ...
    if (user.role === "WORKER" && user.user_id != worker_id) {
      return res
        .status(403)
        .json({ message: "You can only view your own assignments" });
    }
    ...
    if (user.role === "MANAGER") {
      ...
      if (!worker || worker.manager_id !== user.user_id) {
        return res
          .status(403)
          .json({ message: "You can only view assignments of your workers" });
      }
    }
...
export const removeAssignment = async (req, res) => {
  try {
    ...
    const assignment =
      await taskAssignmentService.findAssignmentById(assignment_id);
    if (!assignment) {
    ...
    if (
      ...
      return res
        .status(403)
        .json({
          message: "You can only remove assignments from your projects",
        });
    }
